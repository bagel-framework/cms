<tr>
    <td>
        @if($site->is_home == 1)
            <i class="icon-file-alt is-home"></i>
        @else
    ...
    <td>{{$site->id}}</td>
    <td>
        <a href="{{action('Bagel\Cms\Controllers\SiteController@edit', array('site' => $site->id))}}">{{$site->name}}</a>
    </td>
    <td>{{$site->slug}}</td>
    <td>{{object_get($site->template, 'name', '-')}}</td>
    <td>
